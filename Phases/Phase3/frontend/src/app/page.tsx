...
// Lazy load ChatKit only when needed (reduces initial bundle size)
const ChatKitPanel = dynamic(() => import('@/components/ChatKitPanel'), {
  ssr: false,
  loading: () => (
    <div className="fixed inset-y-0 right-0 w-full sm:w-96 z-50 flex items-center justify-center bg-slate-900 border-l border-slate-800">
...
export default function Home() {
  ...
  const handleAddTask = async (
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : t('errors.failedToCreate');
      setError(message);
      toast.error(message);
    }
  };
  ...
  const handleDeleteTask = async (id: number) => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : t('errors.failedToDelete');
      setError(message);
      toast.error(message);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 relative overflow-hidden">
      ...
      <div className="relative z-10 flex h-screen">
        ...
        <div
          className={`fixed top-0 left-0 bottom-0 z-50 w-56 bg-slate-900 transform lg:hidden transition-transform duration-300 pointer-events-auto overflow-y-auto ${
            ...
            <header className="flex items-center justify-between p-4 lg:p-6 border-b border-slate-800/50">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 px-4 py-2 bg-purple-500/20 text-purple-400 border border-purple-500/30 rounded-lg hover:bg-purple-500/30 hover:border-purple-500/50 transition-all font-medium text-sm"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" />
                  </svg>
                  <span className="hidden sm:inline">{t('chat.aiChat')}</span>
                </button>
      ...
      {/* Reminder Manager - Background alarm service */}
      <ReminderManager tasks={tasks} onTasksUpdated={loadTasks} />
    </div>
  );
}
